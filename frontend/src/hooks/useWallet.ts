import {
  ...
  braavos,
  argent,
  useSwitchChain as useStarknetSwitchChain,
  useNetwork as useSnNetwork,
  useReadContract,
...
import {
  ...
  useSwitchChain as useEVMSwitchChain,
} from "wagmi";
...
export function useWallet() {
  ...
  const {
    data: underlyingStarknetAddress,
    refetch: refetchUnderlyingStarknetAddress,
    isError,
    error,
  } = useReadContract({
    abi: KAKAROT_ABI,
  ...
  const [resolvedStarknetAddress, setResolvedStarknetAddress] = useState<
    `0x${string}` | null
  >(null);
  ...
  const { switchChain: switchStarknetChain, isSuccess: isSnSwitchSuccess } =
    useStarknetSwitchChain({
      params: {
        chainId: `0x${defaultChain.id.toString(16)}`,
      },
    });
  const { switchChain: switchEVMChain, isSuccess: isEvmSwitchSuccess } =
    useEVMSwitchChain();
    ...
    (connector: ExtendedConnector) => {
      if (connector.type === "starknet") {
        const starknetConnector =
          connector.id === "argentX" ? argent() : braavos();
