...
export function useTransaction(walletType: "starknet" | "evm" | null) {
  ...
  const prepareTransactionData = useMemo(
    () => (calls: Call[]) => {
  ...
  const transactionStatus = {
    tx_hash:
      walletType === "starknet"
        ? starknetTransaction.data?.transaction_hash
        : evmData,
    data: walletType === "starknet" ? starknetTransaction.data : evmData,
    isPending:
      walletType === "starknet" ? starknetTransaction.isPending : evmIsPending,
    isError:
      walletType === "starknet" ? starknetTransaction.isError : evmIsError,
    error:
      walletType === "starknet"
        ? starknetTransaction.error?.message
        : evmError?.message,
  };
