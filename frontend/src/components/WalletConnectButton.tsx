...
function ConnectorModal({ connectors, onConnect, onClose }: { connectors: ExtendedConnector[], onConnect: (connector: ExtendedConnector) => void, onClose: () => void }) {
  ...
  const ConnectorGroup = ({ title, connectors, colorScheme }) => (
    <Box borderWidth={1} borderRadius="md" p={4} mb={4} borderColor={`${colorScheme}.200`} bg={`${colorScheme}.50`}>
      <Text fontWeight="bold" mb={2} color={`${colorScheme}.700`}>{title}</Text>
    ...
    <ModalContent maxW="425px">
      ...
      <ModalBody>
        <Box maxH="400px" overflowY="auto" pr={4}>
          <ConnectorGroup title="Starknet Wallets" connectors={starknetConnectors} colorScheme="blue" />
          <ConnectorGroup title="EVM Wallets" connectors={evmConnectors} colorScheme="green" />
        </Box>
      </ModalBody>
      <ModalFooter>
...
function WalletInfo({ address, resolvedStarknetAddress, onDisconnect, walletType }: { address: string, resolvedStarknetAddress: string, onDisconnect: () => void, walletType: 'starknet' | 'evm' }) {
            ...
            <Box mb={4}>
...

              {
                address !== resolvedStarknetAddress && (
                  <Box mb={4}>
                  <Text fontSize="sm" fontWeight="medium" mb={2} color="gray.600">Associated Starknet Address:</Text>
                  <Flex alignItems="center" bg="gray.100" p={3} borderRadius="lg">
                    <Wallet size={20} color="blue.500" />
                    <Text ml={2} fontSize="sm" fontFamily="mono" wordBreak="break-all">{resolvedStarknetAddress}</Text>
                  </Flex>
                </Box>
                )
              }
...
export function WalletConnectButton(): JSX.Element {
  ...
  const renderWalletButton = () => {
    if (isConnected && connectedAddress && walletType) {
      return <WalletInfo address={connectedAddress} resolvedStarknetAddress={resolvedStarknetAddress} onDisconnect={handleDisconnect} walletType={walletType} />;
    }
    return (
